...
public class TarjetacreditoventaJpaController implements Serializable {
    ...
    public void create(Tarjetacreditoventa tarjetacreditoventa) throws RollbackFailureException, Exception {
        ...
        try {
            ...
            for (Pagoventa pagoventaCollectionPagoventa : tarjetacreditoventa.getPagoventaCollection()) {
                Tarjetacreditoventa oldTarjetacreditoidFkOfPagoventaCollectionPagoventa = pagoventaCollectionPagoventa.getTarjetacreditoidFk();
                pagoventaCollectionPagoventa.setTarjetacreditoidFk(tarjetacreditoventa);
                pagoventaCollectionPagoventa = em.merge(pagoventaCollectionPagoventa);
                if (oldTarjetacreditoidFkOfPagoventaCollectionPagoventa != null) {
                    oldTarjetacreditoidFkOfPagoventaCollectionPagoventa.getPagoventaCollection().remove(pagoventaCollectionPagoventa);
                    oldTarjetacreditoidFkOfPagoventaCollectionPagoventa = em.merge(oldTarjetacreditoidFkOfPagoventaCollectionPagoventa);
                }
            }
            utx.commit();
        } catch (Exception ex) {
        ...
        try {
            ...
            for (Pagoventa pagoventaCollectionOldPagoventa : pagoventaCollectionOld) {
                if (!pagoventaCollectionNew.contains(pagoventaCollectionOldPagoventa)) {
                    ...
                    illegalOrphanMessages.add("You must retain Pagoventa " + pagoventaCollectionOldPagoventa + " since its tarjetacreditoidFk field is not nullable.");
                }
            }
            if (illegalOrphanMessages != null) {
            ...
            for (Pagoventa pagoventaCollectionNewPagoventa : pagoventaCollectionNew) {
                if (!pagoventaCollectionOld.contains(pagoventaCollectionNewPagoventa)) {
                    Tarjetacreditoventa oldTarjetacreditoidFkOfPagoventaCollectionNewPagoventa = pagoventaCollectionNewPagoventa.getTarjetacreditoidFk();
                    pagoventaCollectionNewPagoventa.setTarjetacreditoidFk(tarjetacreditoventa);
                    pagoventaCollectionNewPagoventa = em.merge(pagoventaCollectionNewPagoventa);
                    if (oldTarjetacreditoidFkOfPagoventaCollectionNewPagoventa != null && !oldTarjetacreditoidFkOfPagoventaCollectionNewPagoventa.equals(tarjetacreditoventa)) {
                        oldTarjetacreditoidFkOfPagoventaCollectionNewPagoventa.getPagoventaCollection().remove(pagoventaCollectionNewPagoventa);
                        oldTarjetacreditoidFkOfPagoventaCollectionNewPagoventa = em.merge(oldTarjetacreditoidFkOfPagoventaCollectionNewPagoventa);
                    }
                }
            }
            utx.commit();
            ...
            if (msg == null || msg.length() == 0) {
                Integer id = tarjetacreditoventa.getTarjetacreditoventaid();
    ...
    public void destroy(Integer id) throws IllegalOrphanException, NonexistentEntityException, RollbackFailureException, Exception {
        ...
        try {
            ...
            for (Pagoventa pagoventaCollectionOrphanCheckPagoventa : pagoventaCollectionOrphanCheck) {
                ...
                illegalOrphanMessages.add("This Tarjetacreditoventa (" + tarjetacreditoventa + ") cannot be destroyed since the Pagoventa " + pagoventaCollectionOrphanCheckPagoventa + " in its pagoventaCollection field has a non-nullable tarjetacreditoidFk field.");
            }
            if (illegalOrphanMessages != null) {
                throw new IllegalOrphanException(illegalOrphanMessages);
            }
    ...
    public Tarjetacreditoventa findTarjetacreditoventa(Integer id) {
        EntityManager em = getEntityManager();
        try {
